...
{{-- 
    ...
    @forelse ($this->relations as  $data)
        <div class="w-md-50 d-flex justify-content-between align-items-center">
            <p>{{ $data->id_2 }}</p>
            <p class="mx-1">{{ $data->paluser->FULL_NAME_AR }}</p>
            <p class="mx-1">{{ $data->paluser->SEX }}</p>
            <p>{{ myDateStyle1($data->paluser->birth_date) }}</p>
          

            <x-button :label="__('uilogin.add to check')" class="btn-primary" wire:click.prevent='store({{ $data->id_2 }})'></x-button>
        </div>
    @empty
        <p>no data</p>
    @endforelse --}}
    ...
    <div class="col-lg-6 col-xxl-4 order-1 order-xxl-2">
        ...
        <div class="card card-custom card-stretch gutter-b">
            ...
            <div class="card-header border-0">
                <h3 class="card-title font-weight-bolder text-dark">{{__('uilogin.childs')}}</h3>
    ...
    <div class="card-header border-0">
        <h3 class="card-title font-weight-bolder text-dark" style="font-size:1.275rem !important;">{{__('uilogin.added to')}}</h3>
        <div class="card-toolbar">
    ...
    @forelse ($this->childs as $key=> $data)
        <div class="w-md-50 d-flex justify-content-between align-items-center">
            ...
            <x-button :label="__('uilogin.remove from check')" class="btn-danger"
                wire:click.prevent='destroy({{ $data }})'></x-button>
        </div>
    @empty
    <a  class="text-dark text-hover-primary mb-1 font-size-lg mx-1"> {{__('uilogin.no data') }} </a>
    @endforelse
